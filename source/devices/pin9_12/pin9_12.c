...
//
//Pin 9-12 Device File.
//Make a simple device driver that allows user to 
//set, clear, and toggle GPIO Pin9-12 by writing to
//the /dev/pin9_12 file.  
//
...
#define SUCCESS 	0
#define DEVICE_NAME	"pin9_12"	//name as appears in /proc/devices
#define CLASS_NAME	"pin9_12"	//not sure if this needs to be same/diff from device name
...
/////////////////////////////////////////////
//device_init()
//Register pin9_12 device, creating the node in /dev/pin9_12
//Configures Pin9_12 as output and sets the value low
//
static int __init device_init(void)
{
	printk(KERN_INFO "pin9_12.ko - device init()");
	...
	ioOEReg = ioremap(GPIO1_OE_REG, 4);				//read/write?

	//read the current values
	ioDataOutValue = ioread32(ioDataOutReg);		//status of output pins
	ioDataInValue = ioread32(ioDataInReg);			//status of input pins
	ioOEValue = ioread32(ioOEReg);					//OE pin status on GPIO1

	//clear pin 9-12 on OE Register to Set as Output
	iowrite32((ioOEValue &=~PIN_9_12_BIT), ioOEReg);
	iowrite32(PIN_9_12_BIT, ioClearReg);
...
//and the driver.
//Set GPIO Pin 9_12 low and restore to input state
//
static void __exit device_exit(void)
{

	printk(KERN_INFO "pin9_12.ko - device_exit()\n");
	...
	//clear value of pin 9 12
	iowrite32(PIN_9_12_BIT, ioClearReg);
	memset(msg, 0x00, BUF_LEN);
	msgSize = sprintf(msg, "0");

	//set pin 9-12 on OE Register to restore as input
...
static ssize_t device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
	...
	if (error_count == 0)
	{
		...
		switch(msg[0])
		{
			...
			//set
			case '1':
				iowrite32(PIN_9_12_BIT, ioSetReg);
				break;
			//toggle
			case '2':
				if (ioDataOutValue & PIN_9_12_BIT)
